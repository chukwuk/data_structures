...
struct queue_from_stacks* queue_from_stacks_create() {
  struct queue_from_stacks* s = malloc(sizeof(struct queue_from_stacks));
  s->s1 = stack_create();
  s->s2 = stack_create();
  return s;   
}
...
void* queue_from_stacks_front(struct queue_from_stacks* qfs) {
  if (stack_isempty(qfs->s2) != 1) {
     void* temp = stack_top(qfs->s2);
     return temp;
  } 
  else  {
     while (stack_isempty(qfs->s1) != 1) {
      void* temp =  stack_pop(qfs->s1);
      stack_push (qfs->s2, temp);
    }

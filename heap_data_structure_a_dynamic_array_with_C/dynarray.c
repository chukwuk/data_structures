...
struct dynarray* dynarray_create() {
  ...
  da->data = malloc(DYNARRAY_INIT_CAPACITY * sizeof(struct store_data));
  assert(da->data);
  da->size = 0;
...
void _dynarray_resize(struct dynarray* da, int new_capacity) {
   ...
   */
  struct store_data* new_data = malloc(new_capacity * sizeof(struct store_data));
  assert(new_data);
  ...
  for (int i = 0; i < da->size; i++) {
    new_data[i].priority = da->data[i].priority;
    new_data[i].value = da->data[i].value;
  }
...
void re_arr_aft_ins(struct dynarray* da) {
   int parent = ((da->size)-2)/2;
   int child = (da->size)-1;
   int temp; void* temp_v;
   while (parent >= 0) {
      if (da->data[child].priority < da->data[parent].priority) {
          temp = da->data[child].priority;
          temp_v = da->data[child].value;
          da->data[child].priority = da->data[parent].priority;
          da->data[child].value = da->data[parent].value;
          da->data[parent].priority = temp;
          da->data[parent].value = temp_v;
           child = parent;
           parent = (child-1)/2;
     } else {
      break;
...
void re_arr_aft_removal(struct dynarray* da) {
   ...
   int temp; void* temp_v;
   while (left_child<=(da->size-1) || right_child<=(da->size-1) ) {
      if (left_child<=(da->size-1) && right_child<=(da->size-1)) {
          if (dynarray_get_prio(da, left_child) < dynarray_get_prio(da, right_child)) {
              swap_function(da, &parent, &left_child);
              parent = left_child;    left_child = 2*parent+1;  right_child = 2*parent+2;            
          } else {
             swap_function(da, &parent, &right_child); parent = right_child;    left_child = 2*parent+1;  right_child = 2*parent+2;
          }
       } else if (left_child<=(da->size-1) && right_child>(da->size-1)){
          swap_function(da, &parent, &left_child);
          parent = left_child;    left_child = 2*parent+1;  right_child = 2*parent+2;
        } else if (right_child<=(da->size-1) && left_child>(da->size-1) ) {
          swap_function(da, &parent, &right_child); parent = right_child;    left_child = 2*parent+1;  right_child = 2*parent+2;
       } 
}
}
...
void dynarray_insert(struct dynarray* da, void* val, int prio) {
   ...
   */
  da->data[da->size].value = val;
  da->data[da->size].priority = prio;
  da->size++;
  re_arr_aft_ins(da);
}
...
void dynarray_remove(struct dynarray* da) {
  ...
  int temp; void* temp_v;
   temp = da->data[0].priority;
   temp_v = da->data[0].value;
   da->data[0].priority = da->data[(da->size)-1].priority;
   da->data[0].value = da->data[(da->size)-1].value;
   da->data[(da->size)-1].priority = temp;
   da->data[(da->size)-1].value = temp_v;

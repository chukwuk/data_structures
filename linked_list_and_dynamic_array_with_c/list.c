...
struct list* list_create() {
  ...
  h->head = malloc(sizeof(struct link));
  h->head->val = NULL;
  // struct link* second = malloc(sizeof(struct link));
  // struct link* third = malloc(sizeof(struct link));
  h->head->next = NULL;
 // second->next = third;
  //third->next = NULL;
  return h;
...
void list_insert(struct list* list, void* val) {
  ...
  new_link->val = list->head->val;
  new_link->next =  list->head->next;
  list->head->val = val;
